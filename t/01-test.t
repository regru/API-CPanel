...
is( API::CPanel::refs( undef ),   '',       $a);
is( API::CPanel::refs( {} ),      'HASH',   $a );
is( API::CPanel::refs( [] ),      'ARRAY',  $a );
is( API::CPanel::refs( sub {} ),  'CODE',   $a );
is( API::CPanel::refs( \$a ),     'SCALAR', $a );

$a = 'is_success';

ok(! API::CPanel::is_success(), $a);
ok(! API::CPanel::is_success( { error => {}, data => {} } ), $a);
ok(! API::CPanel::is_success( { data  => {} } ), $a);
ok(! API::CPanel::is_success( { } ), $a);
ok(  API::CPanel::is_success( { status => 1 } ), $a);
...
$query = API::CPanel::mk_full_query_string({ host => $test_host, func => 'test', dname => Encode::decode( 'cp1251', 'рег.рф' )});
